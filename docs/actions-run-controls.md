...
Rate-limit and poll headers are safely parsed from every dispatch, including error statuses and unframable responses, and installed into the same per-account floor a read would have installed. Installing that floor admits and releases no read operation: a mutation never uses the automatic read scheduler as its authority, and a mutation never becomes a read. There is no retry-after sleep, no backoff loop, and no automatic resend.
