...
The source layout follows the current pane width. A wide pane shows all three sources side by side. A narrow pane shows one source at a time and keeps the result panel below it. Use the source buttons or Command-Left and Command-Right to change the visible source. Each source scrolls horizontally, so a long line remains available through its final byte instead of wrapping into an unreadable column.

cibergit never writes the conflicted file. Resolve the result in your own editor, then refresh. Opening a source view, changing the selected source, refreshing source stages, and closing the presentation never touch the worktree. Stage resolved result is a separate confirmed command.

The presentation freezes the operation ID, active rebase identity or stash-restore identity, raw path, all three stage objects, and the observed disk generation. If Git replaces a stage, resolves the path, or ends the operation, the old sources stay visible as frozen evidence and staging stops. Refresh changed stages accepts a replacement only for the same operation identity and raw path. It never touches the file on disk. A new operation cannot claim an old presentation just because it conflicts on the same path.

Every conflict, including binary, media, non-UTF-8, missing result, symlink, oversized, rename/delete, and type-change cases, shows source metadata and requires an external tool to resolve the result. The Local changes popover can open the checkout in an application you choose; the rebase panel itself launches nothing, and it never creates, removes, renames, or stages those paths on its own.

Staging re-reads the conflict stages and disk identity, then checks the current Git snapshot guard. Changed source objects stop the confirmation. After staging, the panel reads Git again and marks the presentation resolved only when the path has left the unmerged index.
