...
- Parent `3d51bb3` persists horizontal positions per raw-safe file inside each comparison session, removing cross-mode leakage from the old tab-wide map. Focused file/legacy-record, same-path A→B→A and actual Store round-trip tests plus strict Clippy pass. Latest combined suite at `e53b0b9` has one LocalWorkspace journal test failure: exact mismatch clear encountered a held lock. Unchanged isolated test passes; inherited-descriptor lifetime is under investigation by the current LocalWorkspace owner, not classified as a timeout or cleared by that pass.

Active native BB assignments:

| Worker | Deliverable | Base / workspace / brief |
| --- | --- | --- |
| Sol `thr_64zga8pcf9` | Three-way conflict presentation using the existing result editor; bounded journal-lock investigation | `5494a25`; `/Users/matias/Development/cibergit-three-way-conflicts`; `art_1427c00e-442c-4a41-929b-afb1b8c5487b` / `av_60a41379-ade9-4fa6-9453-5970c8a6289a` |
| Sol `thr_7g9ih6bf57` | Native/inferred/personal stack provenance and proven net remaining diff for an explicit tip | `5494a25`; `/Users/matias/Development/cibergit-stack-net-core`; `art_a9c92635-6d7a-43bf-bae6-3cecfd38e23d` / `av_f50d41d7-094d-412d-8b8e-e5f9fe24f4a1` |
| Grok `thr_ktg6unmzt5` | Independent production-shape correction review | Exact `bba2d9bb`; read-only `/Users/matias/Development/cibergit-pr-lifecycle-shapes`; correction report above |

Consumed workers are archived/stopped. Preserve original preview PID 43840 and its state. All mutation tests use temporary repositories or fake providers; no live remote writes or auth/global changes. Native lifecycle/creation, notifications/sync, conflict/publish/cleanup and final validation remain required. Q60–Q66/Q68 and external signing/physical-input/live-write gates remain explicit; they do not stop independent implementation.
...
| M2-PR | PR lifecycle, metadata, reviewers and checks; F07, R07 | M2 | Accepted provider; G1 for final gate | C0; Grok thr_ktg6unmzt5 | running | Backend shapes e53b0b9 recheck; native UI pending | art_d289a61c-7910-4ea4-afdc-9e565e70edd7 |
...
| M4-STACK | Native/inferred relationships, corrections and aggregate diff; K01-K03, E13 | M4 | G3, M1-SIDEBAR, M2-PROGRESS | Sol thr_7g9ih6bf57 | running | Explicit-tip net backend in progress | Brief above |
...
| M4-CONFLICT | Editable three-way result, external resolution and recovery; B04, B05, E14 | M4 | M4-REBASE, accepted Document backend; native M3-EXTERNAL | Sol thr_64zga8pcf9 | running | Lifecycle651037f accepted; native three-way editor running | art_63cc3dbd-330a-43ae-a827-c4c6fe20b43e |
