...
The stronger parent restart using only `--data-dir` failed before a PR tab became ready. Startup never reopens saved tabs, and a previously opened closed PR can be absent from the cached open list. Failed logs remain in that packet. M1-RESTART worker `thr_vekfhnukui` owns the correction from `d9e1cec` in `/Users/matias/Development/cibergit-workspace-restart`; assignment `art_130f83a9-f8cc-45f5-a32f-b5fc58bd6a7b` / `av_34d09dd2-8242-453d-bf50-fdc9465f7e9b` is verified. Ordinary startup, network-wide offline operation and G1 remain open.

The next independent PR slice is author-owned submitted-review summary editing, assigned to `thr_wv9bzstgfw` from `d9e1cec` in `/Users/matias/Development/cibergit-submitted-review-edit`. Its native confirmation must freeze the exact review, old/new body and account; provider dispatch must revalidate author/capability and preserve uncertainty without replay. Verified assignment `art_7bf84241-1092-47b9-8ee2-8e7cab61bd9f` / `av_fe076a4e-6a6a-4fab-a495-edc07f83e775` follows verified API/source discovery `art_3a187830-dd19-4936-b881-fc37b6ce0cf4` / `av_0e8907a2-b44a-41cf-9031-55b1dcef6534`. File-level comments and dismissal remain follow-on work. No Q65 decision, live mutation or whole-V1 acceptance follows from this dispatch.
...
| M1-SYNC | Polling, explicit revision advancement and offline read; S01-S04, R04, E09 | M1 | M1-DIFF, M1-SIDEBAR | C0; thr_vekfhnukui startup restoration | running | Bounded cache accepted at d9e1cec with explicit-PR native restart; ordinary startup failed and correction active; conditional polling and broader offline proof open | art_3d24796c-a510-478e-b7c3-3291b714912d / av_6a3af1a7-871e-4edf-b4c9-2e13cb29a779; G1 pending |
