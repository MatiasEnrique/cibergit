...
User instruction, 2026-09-21: the type was too heavy — switch to Geist at Regular for headings and Inter for body.
...
Pair two bundled faces: Geist (Regular/Medium/SemiBold) for headings — display, title, subtitle and kicker — and Inter (Regular/Medium/SemiBold) for everything read at 11-13px: body, labels, controls, captions and the sidebar. Geist gives the app its voice at display sizes, where it stays open at Regular; Inter's tall x-height is what keeps the small text legible over translucent chrome. Keep the type light: headings sit at Regular, Medium is for emphasis and SemiBold is the ceiling, and only those three weights are bundled. Keep code aligned in a monospace font. Font files and both SIL Open Font Licenses are in `assets/fonts`; `manifest.json` records the upstream revision or release each file came from, with sizes and SHA-256 checksums. Both families publish static weights upstream, so nothing is derived here. Load the fonts through GPUI’s text system, without installing them globally. Include the font license in app packaging.
