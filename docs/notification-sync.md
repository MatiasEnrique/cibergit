...
Once a response establishes a rate-limit floor, that account's current work stops issuing repository, hydration, and later-chunk requests. Unperformed repositories remain incomplete. The notification panel shows an account-scoped polling-interval or rate-limit notice while the gate blocks; it does not display raw headers or error bodies.

The controller converts relative delays with monotonic time and checked arithmetic. An unrepresentable delay suspends polling instead of shortening the server's minimum. An absolute reset that expires while the response is being processed contributes zero remaining delay and cannot shorten another active floor. A stale selection completion cannot install its cache, snapshot, or alerts. It may still apply a rate-limit delay when the controller lifetime and exact account match, which protects the same credential after a selection change.
