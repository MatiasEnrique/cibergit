...
impl BrowseState {
    ...
    /// Distinct per choice: chips in one row that share an element id stop
    /// answering presses, because the press lands on whichever of them the
    /// framework reached first.
    pub fn element_id(self) -> &'static str {
        match self {
            Self::Open => "browse-state-open",
            Self::Closed => "browse-state-closed",
            Self::All => "browse-state-all",
        }
    }
