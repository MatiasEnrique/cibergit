...
/// The exact tab identity a prepared or confirmed control is bound to.
#[derive(Clone, Debug, PartialEq, Eq)]
pub(super) struct ActionsControlFence {
...
pub(super) struct ActionsControlPreparationToken {
    pub(super) fence: ActionsControlFence,
    pub(super) action: ActionsRunControlAction,
    pub(super) operation_id: String,
    pub(super) attempt_id: String,
}

impl ActionsControlPreparationToken {
    /// A preparation may only land while its own write is still in flight and
    /// the selected check, details generation, and exact run locator are
    /// unchanged.
    pub(super) fn matches(&self, current: &ActionsControlFence) -> bool {
        current.write_in_flight && self.fence == *current
    }
}
...
impl ActionsControlConfirmationToken {
    /// Cancellation and confirmation both check the same fence; only dispatch
    /// completion additionally requires the write to still be in flight.
    pub(super) fn matches(&self, current: &ActionsControlFence, require_in_flight: bool) -> bool {
        if require_in_flight && !current.write_in_flight {
            return false;
        }
        let mut expected = self.fence.clone();
        expected.write_in_flight = current.write_in_flight;
        expected == *current
    }
}
...
mod tests {
    use super::*;
    use cibergit::domain::{
        Account, ActionsRunControlObservation, ActionsRunControlPreparation,
        ActionsRunControlTarget, SelectedViewer,
    };

    fn fence(generation: u64, write_in_flight: bool) -> ActionsControlFence {
        ActionsControlFence {
            workspace_instance: 1,
            tab_instance: 2,
            repository_key: "github/owner/repo".into(),
            pull_request: 7,
            details_generation: 3,
            selected_check_id: Some("CHECK_node".into()),
            locator: None,
            generation,
            write_in_flight,
        }
    ...
    fn request(action: ActionsRunControlAction) -> ActionsRunControlRequest {
            ...
            preparation: ActionsRunControlPreparation {
                ...
                observation: ActionsRunControlObservation {
                    ...
                    viewer: SelectedViewer {
                        node_id: "VIEWER_node".into(),
                        login: "alice".into(),
                    },
                    run_status: "completed".into(),
                    run_conclusion: Some("failure".into()),
                    authority: ActionsRunControlAuthority::Available,
                },
                method: "POST".into(),
                path: "repos/owner/repo/actions/runs/6/rerun".into(),
...
"#,
    ...
    #[test]
    fn a_stale_generation_or_moved_check_cannot_land_a_preparation() {
        ...
        assert!(token.matches(&fence(4, true)));
        assert!(!token.matches(&fence(5, true)));
        assert!(!token.matches(&fence(4, false)));
        let mut moved = fence(4, true);
        moved.selected_check_id = Some("OTHER_node".into());
        assert!(!token.matches(&moved));
        let mut refreshed = fence(4, true);
        refreshed.details_generation = 4;
        assert!(!token.matches(&refreshed));
    }

    #[test]
    fn confirmation_survives_the_idle_tab_but_never_a_new_generation() {
        let token = ActionsControlConfirmationToken {
            fence: fence(9, false),
            request: request(ActionsRunControlAction::RerunAllJobs),
        };
        assert!(token.matches(&fence(9, false), false));
        assert!(token.matches(&fence(9, true), false));
        assert!(!token.matches(&fence(9, false), true));
        assert!(token.matches(&fence(9, true), true));
        assert!(!token.matches(&fence(10, true), true));
    }
